...
const Register = () => {
  ...
  return <div className='bg-bgColor w-full h-[100vh] flex items-center justify-center p-6'>
    <div className='w-full md:w-2/3 h-fit lg:h-full 2x1:h-5/6 py-8 lg:py-0 flex flex-row-reverse bg-primary rounded-xl overflow-hidden shadow-xl'>
      {/*LEFT*/}
      <div className='w-full lg:w-1/2 h-full p-10 2x1:px-20 flex flex-col justify-center'>
        <div className='w-full flex gap-2 items-center mb-6'>
          ...
          <span className='text-2xl text-[#065ad8]' font-semibold>Social</span>
        </div>
        ...
        onSubmit={handleSubmit(onSubmit)}>
         ...
         // labelStyle='ml-2'
          error= {errors.password ? errors.password.message : ""}
            ...
            {/* <Link to='/reset-password' className='rexr-sm text-right text-blue font-semibold'>Forgot Password?</Link> */}
       
        { errMsg?.message && (<span className={`text-sm ${errMsg?.status == "failed" ? "text-[#f64949fe" :
         "text-[#2ba150fe"}mt-0.5`}>{errMsg?.message}</span>)}

         {
          isSubmitting ? <Loading/> : <CustomButton type='submit' containerStyles={`inline-flex justify-center rounded-md bg-blue 
          px-8 py-3 text-sm font-medium text-white outline-none`} title='Create Account'/>
         }
        </form>
      ...
      {/*RIGHT*/}
      <div className='hidden w-1/2 h-full lg:flex items-center jusify-center bg-blue'> {/*flex-col not added*/}
        <div className='relative w-full flex items-center justify-center'>
          ...
          <div className='absolute flex items-center gap-1 bg-white left-10 top-9 py-2 px-5 rounded-full'>{/*top-6*/}
            <ImConnection />
